...
from app.core.process.llm.service import ask_openrouter
from app.features.whisky.collector.prompt.make_whisky_info_promt import make_whisky_prompt, correct_typos_with_llm, \
    extract_whisky_info_with_llm, extract_price_history_with_llm
import json

router = APIRouter(prefix="/llm")

def parse_json_response(response: str) -> str | Any:
    # 백틱(```)과 \n을 제거한 후 JSON 파싱
    cleaned_response = response.replace("```json", "").replace("```", "").strip()
    try:
        # JSON 문자열을 실제 Python 리스트로 파싱
